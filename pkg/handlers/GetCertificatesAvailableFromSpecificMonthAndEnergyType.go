...
func GetCertificatesAvailableFromSpecificMonthAndEnergyType(c *gin.Context) {
	blockhainURL := os.Getenv("BLOCKCHAIN_URL")
	getEnergyCertificateUrl := blockhainURL + "/invoke/channel1/energyTradingBlockchain"

	usableMonth := c.Param("usableMonth")
	usableYear := c.Param("usableYear")
	energyType := c.Param("energyType")
